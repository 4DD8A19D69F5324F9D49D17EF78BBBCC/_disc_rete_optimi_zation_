...
#trivival solver
    ...
    for t in range(1):
        print t
        vec = [ randv(i%warehouseCount)  for i in range(5,int(warehouseCount*10))]
        vec = [ (1e9,v,0) for v in vec if solve2(v,obj,1)[2] != -1 ]
        #vec = [ (1e9,v,0) for v in vec ]
        lvec = len(vec)
        for i in range(800):
            if i%20==0:
                print i
            vl = [ mutate(v[1]) for v in vec]
            lenvl = len(vl)
